...
// 编译期常量：替代 Flutter 的 kDebugMode/kReleaseMode，保证本文件可被纯 Dart 编译
// 语义与 Flutter 对齐：product 构建→release、profile 构建→profile、其余→debug
const bool kReleaseMode = bool.fromEnvironment('dart.vm.product');
const bool kProfileMode = bool.fromEnvironment('dart.vm.profile');
const bool kDebugMode = !kReleaseMode && !kProfileMode;

/// 日志目录解析覆盖点（解耦 path_provider）：
/// App 启动时由 main.dart 注入 path_provider 实现，CLI/测试可注入临时目录。
...
class _HybridOutput extends LogOutput {
  ...
  void output(OutputEvent event) {
    ...
    final isUrgent = level == AppLogLevel.error || level == AppLogLevel.fatal;

    // 1) 文件写入（整条含堆栈，便于 grep 上下文；错误与致命级别立即 flush）
    AppLogFile.writeLine(fullText, urgent: isUrgent);
...
class AppLogFile {
  ...
  static Future<String?> _resolveLogDirViaOverride() async {
    ...
    try {
      final resolver = logDirResolverOverride;
      if (resolver == null) return null;
      final appDirPath = await resolver();
      if (appDirPath == null) return null;
      final candidate = p.join(appDirPath, 'logs');
      if (await _ensureWritableDir(candidate)) return candidate;
    } on Object {
      // 忽略
    }
  ...
  static void writeLine(String line, {bool urgent = false}) {
    if (_dirPath == null) return;
    try {
      _ensureSink();
...
class AppLog {
  ...
  ///   - 非 debug 构建（release/profile）→ info（记录 info/warn/error/fatal，保留关键生命周期与错误上下文）
  ///
  ...
  static void refreshLevel() {
    ...
    _appLogFilter.level = isDevModeActive ? Level.trace : Level.info;
  }
