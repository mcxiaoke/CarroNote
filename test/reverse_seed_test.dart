...
void main() {
  ...
  test('tonalSpot: primary 的 hue 来自 seed，chroma 恒≈36，tone 恒=40(亮)', () {
    final scheme =
        ColorScheme.fromSeed(seedColor: cream, brightness: Brightness.light);
    final p = hct(scheme.primary);
    debugPrint('[cream seed] -> primary '
  ...
  test('seed 的 chroma 完全不影响 primary（只能改 hue）', () {
    ...
    final pLow = hct(ColorScheme.fromSeed(
            seedColor: lowChroma, brightness: Brightness.light)
        .primary);
    final pHigh = hct(ColorScheme.fromSeed(
            seedColor: highChroma, brightness: Brightness.light)
        .primary);
    debugPrint('[low chroma seed]  primary hue=${pLow.hue.toStringAsFixed(1)} chroma=${pLow.chroma.toStringAsFixed(1)}');
    debugPrint('[high chroma seed] primary hue=${pHigh.hue.toStringAsFixed(1)} chroma=${pHigh.chroma.toStringAsFixed(1)}');
  ...
  test('反推 cream 作为 primary：不可行（tone 被钉在 40，chroma 被钉在 36）', () {
    ...
    final produced = ColorScheme.fromSeed(
            seedColor: candidate, brightness: Brightness.light)
        .primary;
    final ph = hct(produced);
    debugPrint('[反推 seed] #${candidate.toARGB32().toRadixString(16)} '
  ...
  test('实证：遍历所有 hue，没有任何 seed 能让 primary 接近奶油', () {
    ...
    final bestPrimary = ColorScheme.fromSeed(
            seedColor: bestSeed!, brightness: Brightness.light)
        .primary;
    final bestLum = bestPrimary.computeLuminance();
    debugPrint('[hue 扫描] 最接近奶油的 primary #${bestPrimary.toARGB32().toRadixString(16)} '
        '与奶油的归一化距离² = ${bestDist.toStringAsFixed(3)} (0=相同, 3=最远)');
    debugPrint('[hue 扫描] 该最近 primary 的 luminance = ${bestLum.toStringAsFixed(3)} '
  ...
  test('实践路径：要真用奶油做主色，必须 copyWith 直接覆盖 primary', () {
    final scheme = ColorScheme.fromSeed(seedColor: cream, brightness: Brightness.light)
        .copyWith(
      primary: cream,
      onPrimary: const Color(0xFF1A1A1A), // 奶油做 primary 时 onPrimary 须用深色保对比
    );
    expect(scheme.primary, equals(cream)); // 覆盖成功
    debugPrint('[copyWith] primary 现在 = #${scheme.primary.toARGB32().toRadixString(16)} '
