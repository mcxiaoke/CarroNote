...
void main() {
  ...
  group('NoteEditorState 保存守卫与内容归一化', () {
    ...
    test('destroyAfter: false 保留状态并将 original 更新为最新已保存笔记', () async {
      ...
      // 首次保存（模拟后台自动保存）
      final firstSaved = await NoteEditorState().addOrUpdateNote(
        destroyAfter: false,
      );
      expect(firstSaved, isNotNull);
      expect(NoteEditorState.original, isNotNull);
      expect(NoteEditorState.original!.uuid, firstSaved!.uuid);

      // 再次编辑同一篇笔记（由于 original 已指向新笔记，这次将作为更新而非新建）
      NoteEditorState.title = '第二次修改';
      final secondSaved = await NoteEditorState().addOrUpdateNote(
        destroyAfter: true,
      );
      expect(secondSaved, isNotNull);
      expect(secondSaved!.uuid, firstSaved.uuid);
      expect(secondSaved.title, '第二次修改');
