...
void main() {
  ...
  group('认证主流程', () {
    testWidgets('首次运行：设置密码 hello.1111 后进入主界面',
      ...
      await tester.enterText(
          find.byType(ShadInputFormField).at(0), kTestPassword);
      await tester.enterText(
          find.byType(ShadInputFormField).at(1), kTestPassword);
      await tester.pump();
    ...
    testWidgets('已有保险库：用 hello.1111 登录后主界面显示已 seed 的笔记',
        (WidgetTester tester) async {
      await prepareUnlockedVault(
        password: kTestPassword,
        seeds: const [
          (title: kSeedNoteTitle, description: kSeedNoteBody),
        ],
      );
      await pumpApp(tester);
      ...
      // 输入密码并登录
      await tester.enterText(
          find.byType(ShadInputFormField), kTestPassword);
      await tester.pump();
    ...
    testWidgets('错误密码：停留在登录页并提示', (WidgetTester tester) async {
      ...
      await tester.enterText(
          find.byType(ShadInputFormField), 'wrong-pass-999');
      await tester.pump();
      await tester.tap(find.widgetWithText(ShadButton, 'Login'));
