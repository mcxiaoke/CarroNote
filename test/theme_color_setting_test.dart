...
void main() {
  ...
  int countSelected(WidgetTester tester) => find.byType(Icon).evaluate().where(
        (e) => (e.widget as Icon).icon == LucideIcons.check,
      ).length;

  /// 取某组的第 [i] 个颜色的英文显示名（用于点击色块；locale 为 en）。
  String colorNameEn(int g, int i) =>
      AppThemeSeeds.itemByIndex(g, i).nameEn;
  ...
  testWidgets('渲染全部分组 tab，进入页面选中当前已应用主题色', (WidgetTester tester) async {
    ...
    expect(countSelected(tester), 1,
        reason: '进入页面应选中当前已应用的主题色（0/0）');
    ...
    expect(applyBtn.onPressed, isNull,
        reason: '未改动时 Apply 按钮应禁用');
  });

  testWidgets('点击色块仅预览，不更新 ThemeProvider；点 Apply 才生效', (WidgetTester tester) async {
    useTallViewport(tester);
    await tester.pumpWidget(wrapScreen(const ThemeColorPicker()));
    await tester.pumpAndSettle();
    ...
    expect(testThemeProvider.colorIndex, 0,
        reason: '点击色块只是预览，不应立即更新 Provider');
    expect(testThemeProvider.seedColor, AppThemeSeeds.colorByIndex(0, 0),
        reason: '预览不应改变当前主题色');
  ...
  testWidgets('Apply 后重进页面，对应 item 保持选中态且主题色不变', (WidgetTester tester) async {
    ...
    expect(countSelected(tester), 1,
        reason: '重进页面应选中已应用的主题色');

    // 无改动时 Apply 禁用
    final applyFinder = find.widgetWithText(ShadButton, 'Apply theme');
    final applyBtn = tester.widget<ShadButton>(applyFinder);
    expect(applyBtn.onPressed, isNull,
        reason: '重进且未改动时 Apply 按钮应禁用');
  });
  ...
  testWidgets('切换分组显示新组颜色，不自动选中同位置颜色', (WidgetTester tester) async {
    ...
    await tester.pumpAndSettle();
    expect(countSelected(tester), 1,
        reason: '进入页面应选中当前已应用主题色（组 0 / i=1）');

    // 切到第 2 组（g=1）：同位置（i=1）颜色不应被自动选中
    await tester.tap(find.text(AppThemeSeeds.groups[1].nameEn));
    await tester.pumpAndSettle();
    expect(find.text(colorNameEn(1, 0)), findsWidgets,
        reason: '切组后网格应展示新组颜色');
    expect(countSelected(tester), 0,
        reason: '切到非当前组时不应自动选中任何颜色（避免同位置误导）');
    // 未选中时 Apply 应禁用
    final applyFinder = find.widgetWithText(ShadButton, 'Apply theme');
    final applyBtn = tester.widget<ShadButton>(applyFinder);
    expect(applyBtn.onPressed, isNull,
        reason: '切组未选色时 Apply 按钮应禁用');

    // 点击新组内的色块（仅预览）→ Apply → 全局更新并持久化
    await tester.tap(find.text(colorNameEn(1, 0)));
    await tester.pumpAndSettle();
    expect(testThemeProvider.groupIndex, currentG,
    ...
    expect(testThemeProvider.colorIndex, 0);
    expect(testThemeProvider.seedColor, AppThemeSeeds.colorByIndex(1, 0),
        reason: 'Apply 后当前主题色应等于所选颜色');
