...
## 开发测试
- 格式: 针对修改过的代码，运行 `dart run import_sorter:main` 和 `dart format` 确保代码格式一致
- 测试: 改代码后运行 `flutter analyze` 和 `dart test packages\core\test` 和 `flutter test` 测试通过
- 验证: 运行 `flutter build windows --debug` 验证编译无错误

## 注意事项
- 翻译语言资源只需要添加 `zh-CN` 和 `en-US` 就行
- 未经用户明确允许，禁止 git commit 和 push
